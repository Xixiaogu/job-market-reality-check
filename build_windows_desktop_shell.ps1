...
& python -c "import webview, pystray, PIL, PyInstaller; print('pywebview', webview.__version__ if hasattr(webview, '__version__') else 'installed'); print('pystray installed'); print('Pillow', PIL.__version__); print('PyInstaller', PyInstaller.__version__)"
if ($LASTEXITCODE -ne 0) {
    Fail "Desktop-shell Python dependencies are missing."
}

Write-Host "Regenerating product icon..." -ForegroundColor Cyan
& python (Join-Path $root "packaging\generate_branding.py")
if ($LASTEXITCODE -ne 0) {
...
try {
    & python -m PyInstaller `
        ...
        --distpath $distRoot `
        --workpath $workRoot `
        (Join-Path $root "job_market_desktop_shell.spec")
